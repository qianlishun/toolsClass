...
@interface QRulerView ()<UICollectionViewDelegate,UICollectionViewDataSource,UIScrollViewDelegate,UICollectionViewDelegateFlowLayout>
...
-(void)setupDataSource{
    ...
    for (int i = 0; i<self.scalesCountBetweenLargeLine; i++) {
        QRulerModel *model = [[QRulerModel alloc]init];
...
-(void)layoutSubviews{
    ...
    if(self.scaleTextFont.pointSize==0){
        self.indicatorView.centerY = CGRectGetHeight(self.frame)/2.0;
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    QRulerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"QRulerCell" forIndexPath:indexPath];
    QRulerModel *model = self.dataArray[indexPath.item % self.scalesCountBetweenLargeLine];
    cell.model = model;
