...
- (void)updateNumberOfVisibleItems
{
    ...
    [_backView.layer.sublayers makeObjectsPerformSelector:@selector(removeFromSuperlayer)];
    
    CAShapeLayer *layer1 = [CAShapeLayer layer];
    CAShapeLayer *layer2 = [CAShapeLayer layer];

    if(_vertical && self.numberOfVisibleItems>0){
        layer1.frame = CGRectMake(0, 0, _backView.width, _backView.height*(1 - 1.0/self.numberOfVisibleItems)/2.0);
        layer2.frame = CGRectMake(0, _backView.height*(0.5 + 1.0/self.numberOfVisibleItems/2.0), _backView.width, _backView.height*(1 - 1.0/self.numberOfVisibleItems)/2.0);
    }else{
        layer1.frame = CGRectMake(0, 0, self.currentItemView.superview.x, _backView.height);
        layer2.frame = CGRectMake(self.currentItemView.superview.x + self.currentItemView.width, 0, _backView.width-(self.currentItemView.superview.x + self.currentItemView.width), _backView.height);
    }
    
    layer1.backgroundColor = _backMaskColor.CGColor;
    layer2.backgroundColor = _backMaskColor.CGColor;
    ...
    if (_scrollEnabled && _numberOfItems)
    {
        switch (panGesture.state)
        {
            ...
            case UIGestureRecognizerStateChanged:
            {
                ...
                velocity /= 2;
                if(_singleScroll){
                    if(translation > _itemWidth) translation = _itemWidth;
                    if(translation < -_itemWidth) translation = -_itemWidth;
                }
                translation /= 2;
#if defined(USING_CHAMELEON) && USING_CHAMELEON
