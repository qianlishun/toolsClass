...
- (void)setVideoURL:(NSURL *)url{
    ...
    self.playerConfiguration.sourceUrl = url;
    [_playbackControls _playerShowPlaybackControls:YES];
    [self _setupPlayer];
}
/**
 初始化播放器
...
- (void)createTimer {
    ...
    self.timeObserve = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 10) queue:nil usingBlock:^(CMTime time){
...
- (SelPlaybackControls *)playbackControls
{
    if (!_playbackControls) {
        _playbackControls = [[SelPlaybackControls alloc]initWithFrame:self.bounds];
...
-(void)videoSliderValueChanged:(SelVideoSlider *)slider{
    ...
    CMTime dragedCMTime = CMTimeMake(dragedSeconds*10, 10);
